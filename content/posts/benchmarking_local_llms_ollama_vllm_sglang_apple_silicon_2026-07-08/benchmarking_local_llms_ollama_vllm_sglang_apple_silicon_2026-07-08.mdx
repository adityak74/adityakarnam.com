...
description: "I set up Ollama, vLLM Metal, and SGLang on an Apple M5 Pro MacBook Pro, ran the same local-serving benchmark harness across all three, and used Gemma 4 as an LLM judge for response quality."
date: 2026-07-08
...
_I wanted a real answer to a practical question: if I am serving local models on my own machine, which stack actually wins for my hardware and workload mix?_

So I built a small benchmark harness, set up **Ollama**, **vLLM Metal**, and **SGLang** on the same Apple Silicon machine, ran the same workload set across all three, and then added a second pass with **Gemma 4 as an LLM judge** for visible response quality.
...
- In the quality pass, **Ollama** won all six workloads because the other two often leaked visible `<think>` traces instead of returning clean final answers

The benchmark harness and report live here:
...
I wanted to benchmark the tasks I personally care about for coding-agent and infra workflows:
...
The config shape in the harness was intentionally simple. Each runtime is just a benchmark target with:
...
If a runtime is fast but returns poor visible answers, the benchmark is incomplete.

So I added a quick **LLM-as-judge** pass using local **`gemma4:31b-mlx`** through Ollama.
...
So for the quality pass, I recollected the Ollama outputs using:
...
That is exactly the kind of operational detail that benchmark charts usually miss.
...
I think this kind of benchmark matters because a lot of agent infrastructure work is quietly moving local again.
...
2. **SGLang** as the experimental or JSON-focused alternative
