...
description: "A close read of the Agent Plugins specification — what the portable core actually standardizes, what it deliberately leaves out, and what it means if you already ship plugins for Claude Code, Codex, or Cursor."
date: 2026-08-10
...
## Migrating is additive, not a rewrite
...
**Read the primary sources:** the [specification](https://agent-plugins.org/specification) (a single readable document — sections 4 through 9 are the substance), the [spec repository](https://github.com/agentplugins/agent-plugins-spec), the [example plugin](https://github.com/agentplugins/agent-plugins-example), and the [Vercel](https://vercel.com/blog/introducing-agent-plugins) and [Google](https://developers.googleblog.com/agent-plugins-package-your-skills-tools-and-more/) announcements.

If you're thinking about agent packaging more broadly, I've written about [what an AI agent harness actually is](/ai-research/what-is-an-ai-agent-harness), [building a minimal harness you can evaluate](/ai-research/building-quecto-from-minimal-harness-to-evaluable-agent), and [running an MCP server as a portfolio interface](/portfolio-mcp-server).
