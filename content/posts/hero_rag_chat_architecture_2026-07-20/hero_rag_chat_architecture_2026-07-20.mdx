...
- **No CORS headers are set on `/api/hero-chat`.** Unlike the [portfolio MCP endpoint](/portfolio-mcp-server/), which needs permissive CORS because external MCP clients call it directly, hero-chat has no `Access-Control-Allow-Origin` header at all — the omission means browsers block cross-origin reads of the response by default, so only same-origin requests from this site can actually use it.
